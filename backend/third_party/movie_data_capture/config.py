...
class Config:
    ...
    def source_folder(self) -> str:
        return self._path_value("common", "source_folder")

    def failed_folder(self) -> str:
        return self._path_value("common", "failed_output_folder")

    def success_folder(self) -> str:
        return self._path_value("common", "success_output_folder")

    def _path_value(self, section: str, option: str) -> str:
        """Return a config path using the current platform's separators.

        Config files are often shared between Windows, macOS and Linux.  A
        relative path remains relative to the process working directory;
        Windows separators are normalized only on POSIX systems.
        """
        value = os.path.expanduser(self.conf.get(section, option).strip())
        if os.name != "nt":
