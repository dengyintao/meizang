...
def download_optional_assets(json_data, path, number, movie_path, multi_part,
                             part, leak_word, c_word, hack_word,
                             fanart_path, poster_path, thumb_path):
    """Download covers and optional assets shared by modes 1 and 3."""
    conf = config.getInstance()
    headers = json_data if 'headers' in json_data else None
    imagecut = json_data.get('imagecut')

    if imagecut == 3:
        small_cover_check(path, poster_path, json_data.get('cover_small'), movie_path, headers)
    image_download(json_data.get('cover'), fanart_path, thumb_path, path, movie_path, headers)

    if multi_part and part.lower() != '-cd1':
        return
    try:
        if conf.is_trailer() and json_data.get('trailer'):
            trailer_download(json_data['trailer'], leak_word, c_word, hack_word, number, path, movie_path)
        if conf.is_extrafanart() and json_data.get('extrafanart'):
            extrafanart_download(json_data['extrafanart'], path, number, movie_path, headers)
        if conf.download_actor_photo_for_kodi():
            actor_photo_download(json_data.get('actor_photo'), path, number)
    except Exception as exc:
        logger.debug('Optional asset download failed: %s', exc, exc_info=True)
...
def core_main(movie_path, number_th, oCC, specified_source=None, specified_url=None):
    ...
    if conf.main_mode() == 1:
        ...
        if multi_part:
            number += part
        download_optional_assets(json_data, path, number, movie_path, multi_part, part,
